  ...
  try {
    ...
    const { error: profileError } = await supabase
      ...
      .update({
        ...
        verified: true, // Also mark user as verified
      })
    ...
    console.log('NIN verification approved for user:', attempt.user_id);

    return new Response(
      JSON.stringify({ success: true, message: 'NIN verification approved' }),
      { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    );
  } catch (error: any) {
    console.error('Error in nin-approve function:', error);
    return new Response(
